...
import { API_BASE_URL, api } from '../../utils/api/client';
...
export const userAPI = {
  ...
  updateCountry: async (countryCode: string) => {
    console.log('更新用户国家/地区:', { countryCode });
    ...
    const response = await fetch(`${API_BASE_URL}/api/private/country`, {
      method: 'PATCH',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      ...
      console.error('更新国家/地区失败响应:', errorData);
      throw new Error(errorData?.detail || errorData?.message || `HTTP ${response.status}`);
    }

    // Check if response is empty (204 No Content) or valid JSON
    if (response.status === 204) {
      console.log('更新国家/地区成功');
      return { success: true };
    }
